...
public class SyncMessageConsumer {
    ...
    private void routePipeline(SyncMessage message) {
        switch (message.getSourceName()) {
            ...
            case "SALESFORCE", "CRM" -> customerPipelineService.runPipelineForJob(message.getJobId(), message.getRecordIds());
            default -> customerPipelineService.runPipelineForJob(message.getJobId(), message.getRecordIds());
        }
    }
}
